...
export function getParserForText(text) {
    ...
    if (!isStrict) {
        ...
        console.log('[Item Importer] Natural language detected, using NaturalItemParser');
        return new NaturalItemParser();
    }

    // Strict template - continue with existing logic
    console.log('[Item Importer] Strict template detected, routing to specialized parser');
    ...
    console.log(`[Item Importer] Strict Dispatcher: Detected item type -> ${itemType}`);

    // Route to specialized parser based on type
    switch (itemType) {
        case 'loot':
            console.log('[Item Importer] Strict Dispatcher: Dispatching to LootStrictParser.');
            return new LootStrictParser();
        case 'tool':
            console.log('[Item Importer] Strict Dispatcher: Dispatching to ToolStrictParser.');
            return new ToolStrictParser();
        case 'container':
            console.log('[Item Importer] Strict Dispatcher: Dispatching to ContainerStrictParser.');
            return new ContainerStrictParser();
        case 'consumable':
            console.log('[Item Importer] Strict Dispatcher: Dispatching to ConsumableStrictParser.');
            return new ConsumableStrictParser();
        case 'weapon':
            console.log('[Item Importer] Strict Dispatcher: Dispatching to WeaponStrictParser.');
            return new WeaponStrictParser();
        case 'equipment':
            console.log('[Item Importer] Strict Dispatcher: Dispatching to EquipmentStrictParser.');
            return new EquipmentStrictParser();
        default:
            console.log('[Item Importer] Strict Dispatcher: No specific parser found. Dispatching to BaseStrictParser.');
            return new BaseStrictParser();
    }
}
